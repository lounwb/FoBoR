...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn=False):
        if attn:
            qs, ks = [], []
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn=False):
        ...
        k_cls = k[:, 0:1, :]  # cls token key, (B, 1, D)
        # v_cls = v[:, 0:1, :]  # cls token value, (B, 1, D)
        # q_cls = q[:, 0:1, :]  # cls token query, (B, 1, D)
        ...
        x = self.ln_post(x[:, 0, :])  # pool

        if self.proj is not None:
            x = x @ self.proj  # global feature
            feat = v @ self.proj  # local feature
        
        # compute patch -> global attn scores, patch token as quey, cls token as key
        if attn:
            # (B, N, D) @ (B, D, 1) -> (B, N, 1)
            scores = (q @ k_cls.transpose(-2, -1)) / math.sqrt(C) # shape: (B, N, 1)
            attn_scores = torch.softmax(scores.squeeze(dim=-1), dim=-1)  # shape: (B, N, 1)

            return x, feat, attn_scores
        
        # x shape: (B, C)
        # feat shape: (B, H_patch*W_patch, C)
        return x, feat
