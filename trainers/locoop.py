...
def entropy_select_topk(p, top_k, label, num_of_local_feature):
    ...
    """
    label_repeat = label.repeat_interleave(num_of_local_feature)  # shape: (B*P, )
    p = F.softmax(p, dim=-1) # shape: (B*P, n_cls)
    pred_topk = torch.topk(p, k=top_k, dim=1)[1] # shape: (B*P, top_k)
